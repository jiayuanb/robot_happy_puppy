function [u1, u2, u3, u4, u5, u6, u7, u8, u9, u10, u11, u12] = coordinator(q1, q2, q3, q4, q5, q6, q7, q8, q9, q10, q11, q12, w1, w2, w3, w4, w5, w6, w7, w8, w9, w10, w11, w12, ss, rr, rl, fr, fl, t, vrr, vrl, vfr, vfl)
    ...
    if cur_time < 0
        ...
        if mod(cur_time, dt) < 0.001
            r = [fl; fr; rl; rr]; % Current foot pos
            rv = [vfl; vfr; vrl; vrr]; % Cuent foot velo
            last_MPC_result = MPC_cntr_wrapper(ss, cur_time, r, dt, N, sd);
            MPC_count = MPC_count + 1;
        end
        if mod(MPC_count,N/2) < 0.001
            [Position_traj_FL, Velocity_traj_FL,d_FL] = FootTraj_forward(fl, ss); % Trajectory & Velocity of FL leg
            ...
            % Currenr foot pos: r; Current foot vel: 
            for i = 1:10 % PD Controll on every segment of trajectory
                [u1,u2,u3,u4,u5,u6,u7,u8,u9,u10,u11,u12] = pd_control_for_leg_swing(Vfoot_d(:,i+1),Pfoot_d(:,i+1),r,vr,ss);
            end
        end
        y = last_MPC_result;
